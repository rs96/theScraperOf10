...
const getAthleteData = async (athleteId) => {
  ...
  const rows = $("#cphBody_pnlPerformances")
      .find(".alternatingrowspanel")
      .first()
      .find("tr");

  // go through each row on the performances table and
  // create a performance object to relate
  // rows.each((i, row) => {
  for (let j = 0; j < 14; j++) {
    const row = rows[j];
    ...
    cells.each((i, cell) => {
      switch (i) {
        ...
        case 10:
          ...
          // if new meeting is added, this is the actual meeting
          // name to add, not the venue id
...
const parsePerformanceFromString = (performance) => {
  let value = 0;
  let rest;
  const splitStrings = performance.split(":");
...
const parseVenueFromString = (venue, tags) => {
  ...
  const newVenueId = venues.length + 1;
  venues.push({
